...
class TopItemDecoration(context: Context) : RecyclerView.ItemDecoration() {
    ...
    override fun onDraw(c: Canvas, parent: RecyclerView, state: RecyclerView.State) {
        ...
        mPaint.apply {
            color = ContextCompat.getColor(mContext, R.color.hi_color_bottom_line)
        }
        textPaint.apply {
            color = ContextCompat.getColor(mContext, R.color.hi_tabtop_dividing_line)
            textSize = 40f
        }
    ...
    override fun onDrawOver(c: Canvas, parent: RecyclerView, state: RecyclerView.State) {
        ...
        if (index != -1) {
            ...
            bottom = childView.bottom.toFloat().coerceAtMost(bottom)
